            ...
            if (currentUser != null) {
                SessionManager.fetchCurrentUserHospitalName { hospitalName: String? ->
                    if (!hospitalName.isNullOrEmpty()) {
                            ...
                            if (!currentUsername.isNullOrEmpty()) {
                                ...
                                assignRef.addValueEventListener(object : ValueEventListener {
                                    override fun onDataChange(snapshot: DataSnapshot) {
                                        ...
                                        if (!snapshot.exists()) {
                                            Log.d("SubmissionsViewModel", "No submissions found at path")
                                        }

                                        for (child in snapshot.children) {
                                            val submissions = child.getValue(Submissions::class.java)
                                            if (submissions != null) {
                                                Log.d("SubmissionsViewModel", "Loaded submissions: ${submissions.submissiontitle} by ${submissions.doctor}")
                                                _submissions.add(submissions)
                                                if (submissions.doctor == currentUsername) {
                                                    _createdSubmissions.add(submissions)
                                                }
                                            } else {
                                                Log.d("SubmittedViewModel", "Failed to parse submission from snapshot: ${child.key}")
                                            }
                                        }
                                        Log.d("SubmissionViewModel", "Total submissions loaded: ${_submissions.size}")
                                        Log.d("SubmissionViewModel", "Created submissions count: ${createdSubmissions.size}")

                                    }

                                    override fun onCancelled(error: DatabaseError) {
                                        Log.e("SubmissionViewModel", "Firebase listener cancelled: ${error.message}")
                                        Toast.makeText(
                                            context,
                        ...
                        Toast.makeText(context, "Failed to get your hospital name", Toast.LENGTH_SHORT).show()
                    }
                }
            }
