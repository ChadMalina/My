...
class UserAuthViewModel(
    ...
    fun signup(
        ...
        if (name.isBlank() || email.isBlank() || medicalcode.isBlank() || password.isBlank() || confpassword.isBlank()) {
            Toast.makeText(
                context,
                "Name, email, medical code, and password cannot be blank",
                Toast.LENGTH_LONG
            ).show()
            return
        ...
        query.get().addOnSuccessListener { snapshot ->
            if (!snapshot.exists()) {
                Toast.makeText(
                    context,
                    "Hospital does not exist in our database",
                    Toast.LENGTH_LONG
                ).show()
                return@addOnSuccessListener
            }

            Toast.makeText(context, "Saving 1", Toast.LENGTH_LONG).show()
            ...
            matchedHospitalName?.let { hospitalName ->
                Toast.makeText(context, "Saving 2", Toast.LENGTH_LONG).show()

                // Create user in firebase Auth
                mAuth.createUserWithEmailAndPassword(email, password)
                    .addOnCompleteListener { task ->
                        Toast.makeText(context, "Saving 3", Toast.LENGTH_LONG).show()
                                ...
                                if (saveTask.isSuccessful) {
                                    ...
                                    if (patientordoctor.equals("Patient", true)) {
                                        navController.navigate(ROUT_PATIENT_DASHBOARD) {
                                            ...
                                            launchSingleTop =
                                                true // prevents creating multiple copies of the same destination
                                        }
                                    } else {
